...
Because calculating pairwise shared count values is memory intensive, this is performed using a random sample of cells, `n_cells` (default=`100`). It may be also be necessary to subset to a group of genes, for example, those with fewer than _n_ counts.
...
df_shared <- setNames(data.frame(Matrix::t(S@sum_sharedcounts[c(2,4),])),c("embedding_1","embedding_2")) / 100
