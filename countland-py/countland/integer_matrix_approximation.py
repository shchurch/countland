...
class IMA_params:
    def __init__(self,rank,u_bounds,l_bounds=[0,0],maxiter=1000000.0,stop_crit=0.0001):
        ...
        self.alpha = 0.01 # this is only used with max inner iter, also could be removed 
...
def IMA_Update_Factor(M,coeff,mkrp,mode,lambda_,params):
    ...
    for k in range(r): # for each of the ranks
        ...
        delta_lambda_k = ( np.dot( M[:,k].T , (mkrp[:,k] - core) ) ) / ( coeff[k,k] * np.power( np.linalg.norm(M[:,k]) , 2 ) )
        ...
        if ( ~(np.isinf(params.l_bounds[mode])) ): # unless lower bound infinite
            Mk[ Mk < params.l_bounds[mode] ] = params.l_bounds[mode] # bound values on low end
        if ( ~(np.isinf(params.u_bounds[mode])) ): # unless upper bound infinite
            Mk[ Mk > params.u_bounds[mode] ] = params.u_bounds[mode] # bound values on high end
        ...
        if(np.all(M[:,k] == 0)):
            ...
            M[np.random.randint(0,M.shape[0],1),k]=1 # add 1 to random value in k-th column
...
def IMA_Compute_Init_Scaled(h,l_bound,u_bound):
    ...
    for j in range(0,h.shape[1]):
        ...
        h[:,j]=np.round(np.multiply(mult_j,h[:,j])) #note subtle difference from how MATLAB rounds 0.5
...
def IMA_init(X,params):
    ...
    for r in range(params.rank):
        ...
        nbig = np.round((1 / params.rank) * X.shape[0]).astype(int) # get 1/R * num cells, rounded
        initU[p[0:nbig],r]=np.random.randint(params.l_bounds[0],params.u_bounds[0]+1,nbig) # grab 1/R*ncells random integers between bounds
    assert np.linalg.matrix_rank(initU)==params.rank, "initializing U failed to achieve target rank, try again"
    
    rank_flag = False # keep trying until rank == R
    max_rank_flag = 0
    while rank_flag == False:
        pat_picked=np.random.randint(0,X.shape[0],params.rank) # random integers 1 to num cells, choose R
        initV = X[pat_picked,:].T # pull random cells, transpose
        initV = IMA_Compute_Init_Scaled(initV,params.l_bounds[1],params.u_bounds[1]) # rescale to within bounds
...
def IMA(X,params):
    ...
    assert (sum(lambda_ >= 1) == len(lambda_)) # check that the sum of lambda >= 1 is the length of lambda
    ...
    while iter_ <= params.maxiter:
        ...
        # Check convergence
        e = np.append(e,np.sqrt(np.max(nX - (2*np.sum(np.sum(np.multiply(V_,A)))) + np.sum(np.sum(np.multiply(B,(np.dot(V_.T,V_))))),0)))
        fit = np.append(fit,1 - (e[-1] / np.sqrt(nX)))

        if (iter_ > 0):
            if (np.absolute(fit[-2] - fit[-1]) < params.stop_crit): # check if last step showed little change, below threshold
